...
class Scene(BaseScene):
    def message_handler(self, event):
        ...
        if text != MESSAGE_KEYBOARD['back_keyb']:
            ...
            elif text == MESSAGE_KEYBOARD['settings_city']:
                data.scens = 'settings_city'
                data.save()
                self.vk_bot.messages.send(peer_id=event.object.peer_id, keyboard=self.keyboard.keyboard_city(), message=self.lang["settings_city_msg"], random_id=0)
